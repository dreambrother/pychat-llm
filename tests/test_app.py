...
from pychat_llm.domain import ChatMessage
...
class TestHistoryPersistence:
    @pytest.mark.asyncio
    async def test_history_saved_on_app_unmount(self, mock_llm):
        ...
        assert len(history_service._history) >= 1, f"Expected at least 1 saved chat, got {len(history_service._history)}"
        title, messages = history_service.get_chat()
        assert any("First message" in msg.text for msg in messages)

    @pytest.mark.asyncio
    async def test_subsequent_messages_appended_to_same_chat(self, mock_llm):
        history_service = HistoryService()
        app = ChatApp(history_service=history_service, llm_provider=mock_llm)
        ...
        assert len(history_service._history) == 1, f"Expected 1 saved chat, got {len(history_service._history)}: {list(history_service._history.keys())}"
        title, messages = history_service.get_chat()
...
class TestHistoryList:
    @pytest.mark.asyncio
    async def test_history_list_displays_saved_chats(self, mock_llm):
        history_service = HistoryService()
        history_service._history["test_chat"] = [
...
class TestChatLoading:
    @pytest.mark.asyncio
    async def test_chat_loaded_from_history(self, mock_llm):
        history_service = HistoryService()
        history_service._history["existing_chat"] = [
            ChatMessage(id=1, text="Welcome", is_user=False, created_at=datetime.now()),
